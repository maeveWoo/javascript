...
```
프로퍼티에 이벤트 핸들러를 할당하면, 하나의 이벤트에 둘이상의 핸들러를 붙일 수 없다.
...
```
속성의 기본적인 특성에 따라 이후에 속성을 설정하여면 이전 속성을 덮어쓴다. 이벤트 핸들러도 마찬가지.
...
- 그다음, <html> 내부의 다음 요소로 이동한다, 그리고 같은 동작을 
...
> event.tartget 이벤트의 대상이었던 요소를 가져오는 데 사용하고 있다. 이 이벤트를 처리한 요소에 엑세스 하려면 event.currentTarget 고
